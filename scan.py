...
class x_black_folders(FFilter):
    _lst=list()

    def __init__(self, name='', lst_folders=list()):
        self.name = name
...
def main():
    ...
    black_dirs=x_black_folders(name='excl. dirs', lst_folders=[r'facebook\images', r'd:\proba\Yandex\2011\sc', r'P\Photos (2)\worked', 'Подшипник Мебиуса _ Иллюзии_files'])
    ...
    f_lst=[black_list, white_list, black_dirs, black_names, white_names, x_archive]

    for f in f_lst:
        strF=str(f)
        print(strF)

    lstf.filters(black_list, white_list, black_dirs, black_names, white_names, archive)
