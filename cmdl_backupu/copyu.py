...
from cmdl_backupu import actions


def dir_path(path, exist=True):
    p = Path(path)
    if p.is_dir():
        return path
    else:
        raise argparse.ArgumentTypeError('{path} must be a valid existing path'.format(path=path))


def parse_args():
    parser = argparse.ArgumentParser(description='Copy files from SOURCE to DESTINATION',
                                     prog='copyu.py',
                                     epilog="(c) CMASF, g.golyshev@forecast.ru", fromfile_prefix_chars='@',
                                     prefix_chars='-+')
    parser.add_argument('-n', '--name', help='name for work', required=False, default='copy_work')
    parser.add_argument('-s', '--source', help='Source dir (one) for copy or move', required=True, type=dir_path)
    parser.add_argument('-d', '--destination', help='Target dir (one) for copy or move', required=True)
...
def main():
    ...
    if args.only_extensions:
        filters.append(scan.x_extension_list(name='only ext', lst_ext=make_param_list(args.only_extensions)),
                       black=False)
    ...
    x._copy_files(work_type=w, zip=z, log_level=log_levels[log_level])
...
def copyz():
    prs = parse_args()
    args = prs.parse_args(['-n', 'test', '-s', r"/home/egor/git/jupyter", '-d', "/home/egor/T"])
    print(Path(vars(args)['source']).resolve())
